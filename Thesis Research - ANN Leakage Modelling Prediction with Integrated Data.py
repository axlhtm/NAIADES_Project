#  ==============================================================================================
#  THESIS RESEARCH - ARTIFICIAL NEURAL NETWORKS LEAKAGE MODELLING PREDICTION WITH INTEGRATED DATA
#  ==============================================================================================
...
df  = pd.read_excel(r'G:/My Drive/College Data/S2 - M.Sc. - Hydroinformatics - UN-IHE Delft/Thesis Research/Data CUP Braila/Acoustic Data/Dataset All Output - Integrated.xlsx', sheet_name='Data Integration - WO Norm')
## II.A.SELET FEATURES INSIDE THE DATASET 
df = df.drop(df.columns[1], axis = 1) # Delete Mean - TD Feature
#df = df.drop(df.columns[1], axis = 1) # Delete Peak - TD Feature
#df = df.drop(df.columns[2], axis = 1) # Delete Peak - FD Feature 
df = df.drop(df.columns[3], axis = 1) # Delete Kurtosis - FD Feature
#df = df.drop(df.columns[4], axis = 1) # Delete Pressure w/o Leak - Hydraulic Feature
#df = df.drop(df.columns[5], axis = 1) # Delete Pressure w/ Leak - Hydraulic Feature
#df = df.drop(df.columns[-3], axis = 1) # Delete Delta Pressure Leak - Hydraulic Feature
